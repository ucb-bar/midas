...
Target reset tokens are presented through `tReset` as `DecoupledIO`. Thus, define **target fire** with `io.hPort.toHost.hValid`, `io.hPort.fromHost.hReady`, `io.tReset.valid`, and additional timing conditions if necessary. Then, the tokens from the widget are generated by connecting **target fire** to `io.hPort.fromHost.hValid`. In the same way the tokens toward the widget are consumed by connecting **target fire** to `io.hPort.toHost.hReady` and `io.tReset.ready`.
...
Finally, generate memory-mapped registers for the transaction buffers and control signals with the following functions:
```scala
...
class MyZynqConfig extends Config(new WithMidasTopEndpoints ++ new midas.ZynqConfig)
```
