...
class PCRAMMMRegIO(cfg: PCRAMConfig) extends SplitTransactionMMRegIO(cfg: BaseConfig){
...
class PCRAMIO(cfg: PCRAMConfig)(implicit p: Parameters)
    extends SplitTransactionModelIO()(p) {
  val mmReg = new PCRAMMMRegIO(cfg)
}

class PCRAMBankQueueEntry(cfg: PCRAMConfig)(implicit p: Parameters) extends Bundle {
  val xaction = new TransactionMetaData
  val bankAddr = UInt(log2Ceil(cfg.maxBanks).W)
  override def cloneType = new PCRAMBankQueueEntry(cfg)(p).asInstanceOf[this.type]
}

// Appends a target cycle at which this reference should be complete
class PCRAMReference(cfg: PCRAMConfig)(implicit p: Parameters) extends Bundle {
...
object PCRAMConstants {
  val nBankStates = 3
  val bankIdle :: bankBusy :: bankPrecharge :: Nil = Enum(nBankStates)
}

import PCRAMConstants._

class PCRAMModel(cfg: PCRAMConfig)(implicit p: Parameters) extends SplitTransactionModel(cfg)(p) {
  ...
  val newReference = Wire(Decoupled(new PCRAMReference(cfg)))
