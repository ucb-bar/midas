...
object Compiler {
  def apply(chirrtl: Circuit,
    ...
    val result = new Compiler(confFile, jsonFile, lib getOrElse jsonFile, macroFile, pathFile) compile (
      CircuitState(chirrtl, ChirrtlForm, Some(new AnnotationMap(targetAnnos ++ replayAnnos))), verilog)
  ...
  def apply[T <: chisel3.core.UserModule](
    ...
    val chirrtl = Parser.parse(chisel3.Driver.emit(circuit))
    val io = dut.getPorts map (_.id)
    apply(chirrtl, circuit.annotations.toSeq, io, dir, lib)
  }
}
